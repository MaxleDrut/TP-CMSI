...
const float nivMer = -0.82; //y en desosus duquel c'est l'océan
...
{
    float rayon = 40.0;
    vec2 centreSol = iResolution.xy/2.0;
    float coefAppla = iResolution.x/iResolution.y;
    float distCentre = 180.0;
    
  	centreSol.x += cos(iTime)*coefAppla*distCentre;
    centreSol.y += sin(iTime)*distCentre;
    
    vec2 centreLune = iResolution.xy/2.0;
    centreLune.x += -cos(iTime)*coefAppla*distCentre;
    centreLune.y += -sin(iTime)*distCentre;
    
    float distSol = sqrt(pow(fragCoord.x-centreSol.x,2.0) + pow(fragCoord.y-centreSol.y,2.0));
	float distLune = sqrt(pow(fragCoord.x-centreLune.x,2.0) + pow(fragCoord.y-centreLune.y,2.0));
    
    if(distSol<=rayon) {
     	vec3 rgb = vec3((255.0-11.0*(distSol/rayon))/255.0,(255.0-11.0*(distSol/rayon))/255.0,(255.0-181.0*(distSol/rayon))/255.0); 
        return rgb;
    } else if(distLune<=rayon) {
        vec3 rgb = vec3(0.90,0.90,0.90);
        vec2 plan = vec2(distLune/rayon,distLune/rayon);
       
        rgb = rgb + 0.2*turbulence(plan,1.0,0.01, 0.4, 10);
        return rgb;
        
    } else {
    	float facteurJour = 1.0;
    	if(sin(iTime)<0.2) {
        	facteurJour = sin(iTime)+0.6;
    	}
...
vec3 Shade(vec3 p, vec3 n, int s, float deltaY)
{
   ...
   vec3 c;
       ...
       float time = iTime; 
		...
		vec3 lightPos;
        if(sin(time)>=0.0) {
          lightPos = vec3(100000.0*cos(time),sin(time)+0.5,100000.0);
          coefNuit = 0.6*sin(time)+0.4;
        } else {
          coefNuit = 0.4+sin(time)*0.2;
          lightPos = vec3(100000.0*cos(time),sin(time),100000.0);
        }
